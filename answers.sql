...
SELECT models.name AS model_name, brands.name AS brand_name, headquarters FROM brands JOIN models USING (brand_id) WHERE models.name = 'Mustang';
...
SELECT models.year, models.name, model_id, awards.name FROM models LEFT JOIN awards ON awards.winner_id = models.model_id WHERE models.year > 1963;
...
SELECT b.name, b.founded FROM brands AS b FULL OUTER JOIN models AS m ON m.brand_id = b.brand_id WHERE model_id IS NULL;
...
SELECT brands.brand_id FROM brands JOIN models ON models.brand_id = brands.brand_id GROUP BY brands.brand_id HAVING COUNT(*) > 5;
...
SELECT models.name, models.year FROM models WHERE models.year IN (SELECT founded FROM brands); 